...
logging.basicConfig(level=logging.DEBUG,
                ...
                datefmt='%m-%d %H:%M',
                #filename=r'E:\FlaskPyform\aitracktive.log',
...
@app.route("/livetrackrecognition", methods=['GET', 'POST'])
def live():    
    # Send template information
    return render_template('livetrackrecognition.html')
    ...
    if request.method =='GET': # happens when I click on continue
        ...
        if mineral == 'laft':                
            ...
            if roi == 'polygon':
                ...
                polygoncoords_split = polygoncoords.split(',')
                polygoncoordslist = list()
                # Get list of strings
                for i in polygoncoords_split:
                    i = i.replace('[','')
                    i = i.replace(']','')
            ...
            # Variables
            if roi =='polygon':
                polygon = polygoncoords # imported list of lists (from the javascript canvas)
            else:
                polygon = polygon_output  # list of tuples
            ...
            d = {'Name': ['Method', 'Name apatite', 'Name mica', 'Area', 'Ns', 'Ni', 'Polygon', 'Spont tracks: true positives', 'Spont tracks: false negatives', 'Spont tracks: false positives', 'Induced tracks: true positives' , 'Induced tracks: false negatives', 'Induced tracks: false positives', 'DNN apatite', 'DNN mica', 'apprecision', 'aprecall', 'micaprecision', 'micarecall'], 
                'Value': ['Apatite',filename, 'n/a', round(float(area)), ns, ni, polygon, stp, sfn, sfp, itp, ifn, ifp, dnnap, dnnmica, 'apprecision', 'aprecall', 'n/a', 'n/a'],
                }
            ...
            # Check the polygon list again
            logging.info('polygon')
            logging.info(polygon)

            # Return to the annotation window 
            logging.info('render template')
            return render_template('aitracktiveannotate.html', filename = filename, filenamez = filenamez, filenameepi = filenameepi, mineral = mineral, autotracks = stp, area = round(area), polygonsending = polygon, roi = roi)
        ...
        # If only mica needs to be analyzed
            ...
            # Make dictionary with names and values
            d = {'Name': ['Method', 'Name apatite', 'Name mica', 'Area', 'Ns', 'Ni', 'Polygon', 'Spont tracks: true positives', 'Spont tracks: false negatives', 'Spont tracks: false positives', 'Induced tracks: true positives' , 'Induced tracks: false negatives', 'Induced tracks: false positives', 'DNN apatite', 'DNN mica', 'apprecision', 'aprecall', 'micaprecision', 'micarecall'], 
                'Value': ['Apatite','n/a', filename_mica, round(float(area)), ns, ni, polygon, stp, sfn, sfp, itp, ifn, ifp, dnnap, dnnmica, 'apprecision', 'aprecall', 'n/a', 'n/a'],
                }
            ...
            # Return to the annotation window 
            logging.info('render template')
            return render_template('aitracktiveannotate.html', filename = filename_mica, filenamez = filename_micaz, filenameepi = filename_mica_epi, mineral = mineral, autotracks = itp, area = round(area), polygonsending = polygon, roi = roi)
        ...
        # ---------------------------------------------------------------------------------------
            ...
            number_of_auto_tracks = 0
            polygon = 'NaN'
            return render_template('aitracktiveannotate.html', filename = filename_mica, filenamez = filename_micaz, filenameepi = filename_mica_epi, mineral = mineral, autotracks = number_of_auto_tracks, area = round(area), polygonsending = polygon, method=mineral)      
...
def aitracktive2ndpage(filename):
    ...
    if request.method =='GET': # happens when I click on continue
        ...
        if mineral == 'laft':
            ...
            logging.info(len(false_positives))
        ...
        # If we're only analysing fission tracks in mica, I called it 'glass'
            ...
            # Make dictionary with names and values
            d = {'Name': ['Method', 'Name apatite', 'Name mica', 'Area', 'Ns', 'Ni', 'Polygon', 'Spont tracks: true positives', 'Spont tracks: false negatives', 'Spont tracks: false positives', 'Induced tracks: true positives' , 'Induced: false negatives', 'Induced: false positives', 'DNN apatite', 'DNN mica', 'apprecision', 'aprecall', 'micaprecision', 'micarecall'], 
                'Value': ['Apatite','n/a', filename, round(float(area)), ns, ni, polygon, stp, sfn, sfp, itp, ifn, ifp, dnnap, dnnmica, 'n/a', 'n/a', micaprecision,micarecall],
                }
